    ...
    <Card className={cn('py-0', discouraged && 'bg-warn/5')}>
      <CardContent className="grid gap-2 py-4">
        <div className="flex items-baseline gap-2">
          <span className={cn('font-display text-[15px] font-semibold', discouraged ? 'text-warn' : 'text-brand')}>{c.name}</span>
          {c.suffix && <span className="text-[11.5px] text-muted-foreground">{c.suffix}</span>}
        </div>
          ...
          {c.versions.map((v, i) => (
            <div key={i} className="grid gap-0.5">
              <div className="flex items-start gap-1.5 text-[12.5px]">
                ...
                {v.recommended && <Badge variant="secondary" className="ml-1 h-4 gap-1 bg-ok/15 px-1.5 text-[9.5px] text-ok"><Check className="size-2.5" /> recommended</Badge>}
              </div>
              {v.notes.map((nt, j) => (
                <p key={j} className="ml-5 text-[11.5px] leading-normal text-muted-foreground">{nt}</p>
              ))}
            </div>
          ))}
        </div>
    ...
    <div className="grid gap-5">
      ...
      {data.allowed.length > 0 && (
        <section className="grid gap-3">
          <h2 className="font-display text-[15px] font-semibold">Allowed</h2>
        ...
        <section className="grid gap-3">
          ...
          <Card>
            <CardContent>
              <RichHtml
                html={data.notesHtml}
                className="text-[13px] [&_a]:text-brand [&_a]:underline [&_li]:ml-4 [&_li]:mb-1.5 [&_ul]:list-disc [&_ul_ul]:mt-1 [&_.error_red]:font-medium [&_.error_red]:text-warn"
              />
            </CardContent>
          </Card>
        </section>
